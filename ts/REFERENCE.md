# DatagovCkan TypeScript SDK Reference

Complete API reference for the DatagovCkan TypeScript SDK.
...
```ts
new DatagovCkanSDK(options?: object)
```
...
#### `DatagovCkanSDK.test(testopts?, sdkopts?)`
...
**Returns:** `DatagovCkanSDK` instance in test mode.
...
| `facets` | `Record<string, any>` | No | Facet results |
...
| `organization` | `Record<string, any>` | No | Organization information |
...
Return the parent `DatagovCkanSDK` instance.
