...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.DATAGOVCKAN_TEST_LIVE ||
    'TRUE' === process.env.DATAGOVCKAN_TEST_OVERRIDE
  ) {
  ...
  m.DATAGOVCKAN_TEST_EXPLAIN = process.env.DATAGOVCKAN_TEST_EXPLAIN || m.DATAGOVCKAN_TEST_EXPLAIN
