...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.DATAGOV_CKAN_TEST_LIVE ||
    'TRUE' === process.env.DATAGOV_CKAN_TEST_OVERRIDE
  ) {
  ...
  m.DATAGOV_CKAN_TEST_EXPLAIN = process.env.DATAGOV_CKAN_TEST_EXPLAIN || m.DATAGOV_CKAN_TEST_EXPLAIN
