...
//     (\`new DatagovCkanSDK(...)\` / \`DatagovCkanSDK.test(...)\` become
//     \`DatagovCkanSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"dataset":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DatagovCkanSDK(...)\` and any
// \`DatagovCkanSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DatagovCkanSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DatagovCkanSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
