...
class DatagovCkanTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DatagovCkanTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DatagovCkanTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DatagovCkanTestRunner.getenv("DATAGOV_CKAN_TEST_LIVE")
        override = DatagovCkanTestRunner.getenv("DATAGOV_CKAN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DatagovCkanTestRunner.getenv(key)
        ...
        explain = DatagovCkanTestRunner.getenv("DATAGOV_CKAN_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if DatagovCkanTestRunner._test_control is not None:
            return DatagovCkanTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DatagovCkanTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DatagovCkanTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DatagovCkanTestRunner.load_test_control()
...
def load_env_local():
    DatagovCkanTestRunner.load_env_local()


def env_override(m):
    return DatagovCkanTestRunner.env_override(m)


def entity_data(v):
    return DatagovCkanTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DatagovCkanTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DatagovCkanTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DatagovCkanTestRunner.load_test_control()


def live_delay_ms():
    return DatagovCkanTestRunner.live_delay_ms()
