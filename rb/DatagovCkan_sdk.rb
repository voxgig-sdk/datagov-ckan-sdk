...
require_relative 'DatagovCkan_types'


class DatagovCkanSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = DatagovCkanHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = DatagovCkanHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, DatagovCkanFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    DatagovCkanUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = DatagovCkanHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = DatagovCkanHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = DatagovCkanHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => DatagovCkanError.new(
        "#{op}_allow",
        "DatagovCkanSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = DatagovCkanHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = DatagovCkanError.new(
        "graphql_error", "DatagovCkanSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = DatagovCkanSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
