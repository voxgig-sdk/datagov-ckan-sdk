# DatagovCkan SDK Dataset entity
...
class DatasetEntity
  ...
  def data_set(args)
    if args
      @_data = DatagovCkanHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = DatagovCkanHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Dataset, Hash] the loaded Dataset; raises DatagovCkanError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DatagovCkanHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
