// Command DatagovCkan-cli is a CLI + REPL for the DatagovCkan SDK,
...
const prompt = "datagov-ckan"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "dataset"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DATAGOV_CKAN_APIKEY (api key), DATAGOV_CKAN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DATAGOV_CKAN_APIKEY carries the API key and
	// DATAGOV_CKAN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DATAGOV_CKAN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DATAGOV_CKAN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDatagovCkanSDK(opts)
