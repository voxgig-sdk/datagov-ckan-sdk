...
boru-driven command-line client **and** interactive REPL for the DatagovCkan
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/datagov-ckan-cli)
make build

# 2. See usage (words, entities, env vars)
./datagov-ckan-cli --help

# 3. Provide credentials once, via the environment
export DATAGOV_CKAN_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./datagov-ckan-cli load 1 dataset            # {id:1} shorthand
./datagov-ckan-cli load '{id:1}' dataset       # explicit match map

# 5. Override the API base URL for a single call
DATAGOV_CKAN_BASE=https://api.example.com ./datagov-ckan-cli load 1 dataset

# 6. No arguments -> interactive REPL
./datagov-ckan-cli
datagov-ckan> load 1 dataset
datagov-ckan> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/datagov-ckan-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/datagov-ckan-cli load 1 dataset
   ```
...
```sh
./datagov-ckan-cli load 1 dataset          # scalar shorthand for {id:1}
./datagov-ckan-cli load '{id:1}' dataset     # explicit match map
```
...
./datagov-ckan-cli load 1 dataset
```
...
Run with no arguments to open a REPL (prompt `datagov-ckan>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/datagov-ckan-cli`. |
...
(`./datagov-ckan-cli <expr>`) and interactively (the REPL), and expressions compose the
