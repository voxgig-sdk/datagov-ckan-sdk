...
[MCP](https://modelcontextprotocol.io) server exposing the DatagovCkan SDK as
two agent tools — `datagov-ckan_list` and `datagov-ckan_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/datagov-ckan-mcp)
make build

# 2. Provide credentials via the environment
export DATAGOV_CKAN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user datagov-ckan \
  -- /absolute/path/to/datagov-ckan-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./datagov-ckan-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// datagov-ckan_list: first page of records
{ "entity": "dataset" }
{ "entity": "dataset", "query": { } }

// datagov-ckan_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/datagov-ckan-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user datagov-ckan \
     -- "$PWD"/dist/*/datagov-ckan-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `datagov-ckan_list` and `datagov-ckan_load` tools now appear
   in new sessions. Ask the agent to *"list dataset using datagov-ckan"*
   and it calls `datagov-ckan_list` with `{"entity":"dataset"}`.
...
```sh
export DATAGOV_CKAN_APIKEY=sk_live_xxx            # API key
export DATAGOV_CKAN_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `datagov-ckan_list` tool
...
### Call the `datagov-ckan_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `datagov-ckan_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `datagov-ckan_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DATAGOV_CKAN_APIKEY` | API key sent with every request. |
| `DATAGOV_CKAN_BASE` | Optional override of the API base URL. |
...
```sh
./datagov-ckan-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"datagov-ckan_load","arguments":{"entity":"dataset","query":{"id":1}}}}'
```
