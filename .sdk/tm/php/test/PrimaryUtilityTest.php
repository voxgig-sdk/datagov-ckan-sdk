...
// DatagovCkan SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        DatagovCkanSDK $client,
        DatagovCkanUtility $utility
    ): DatagovCkanContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new DatagovCkanContext($ctxmap, null);
    ...
    private static function fixctx(DatagovCkanContext $ctx, DatagovCkanSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?DatagovCkanError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new DatagovCkanError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new DatagovCkanResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new DatagovCkanBaseFeature();
    ...
    {
        $hook_client = DatagovCkanSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends DatagovCkanBaseFeature {
            public $hook_fn = null;
            public function TestHook(DatagovCkanContext $ctx): void
            {
    ...
    {
        $init_client = DatagovCkanSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends DatagovCkanBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new DatagovCkanSDK([
            'system' => [
    ...
    {
        $blocked_client = new DatagovCkanSDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new DatagovCkanSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = DatagovCkanSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new DatagovCkanOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = DatagovCkanSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof DatagovCkanError) ? $result->err->msg : (string)$result->err;
