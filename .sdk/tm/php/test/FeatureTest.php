...
class FtCtrl extends DatagovCkanControl
{
...
class FtClient
{
    ...
    private DatagovCkanUtility $utility;

    public function __construct(DatagovCkanUtility $utility, array $options)
    {
    ...
    public function get_utility(): DatagovCkanUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DatagovCkanContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DatagovCkanContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DatagovCkanContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DatagovCkanConfig::make_feature($name);
    ...
    private static function buildUrl(DatagovCkanSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DatagovCkanError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DatagovCkanResponse($fetched);
        }
    ...
    private function fail(DatagovCkanContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DatagovCkanContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
