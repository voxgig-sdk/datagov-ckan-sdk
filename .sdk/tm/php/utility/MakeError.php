...
class DatagovCkanMakeError
{
    public static function call(?DatagovCkanContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new DatagovCkanContext([], null);
        }
        $op = $ctx->op ?? new DatagovCkanOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new DatagovCkanResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof DatagovCkanError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new DatagovCkanError('', $msg, $ctx);
        ...
        if ($err instanceof DatagovCkanError) {
