...
module DatagovCkanUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DatagovCkanResponse.new({})
    ...
    if fetch_err
      ...
      response = DatagovCkanResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DatagovCkanResponse.new(fetched)
    else
